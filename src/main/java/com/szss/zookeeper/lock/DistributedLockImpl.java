...
import com.szss.zookeeper.ZkConstants;
import com.szss.zookeeper.ZooKeeperClient;
import com.szss.zookeeper.ZookeeperUtil;
...
public class DistributedLockImpl implements IDistributedLock {
    ...
    public void doBeforeLock() throws InterruptedException, KeeperException,
        ...
        currentNode = zkClient.get().create(
                lockPath + ZkConstants.ZK_BROADA_LOCK_MARKUP, null, acls,
        ...
        try {
            ...
            if (stat != null) {// 删除锁标记
                zkClient.get().delete(currentNode,
                        ZkConstants.ZK_ZNODE_DATA_ALLVERSION);
            } else {// 锁标记不存在
